...
for T = 1:size(rawFiles,1)
    ...
    for f = 1:size(bData,3)
        if f == 1
            imwrite(bData(:,:,f),['BlueFrames_',num2str(T),'.tif']);
            imwrite(vData(:,:,f),['HemoFrames_',num2str(T),'.tif']);
        else
            imwrite(bData(:,:,f),['BlueFrames_',num2str(T),'.tif'],'WriteMode','append');
            imwrite(vData(:,:,f),['HemoFrames_',num2str(T),'.tif'],'WriteMode','append');
        end
    end
